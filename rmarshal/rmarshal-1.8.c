...
static ID s_dumpas, s_dump, s_load, s_mdump, s_mload;
    ...
    int limit;
{
	...
	if (rb_respond_to(obj, s_dumpas)) {
	    VALUE v;

	    v = rb_funcall(obj, s_dumpas, 0, 0);
	    w_object(v, arg, limit);
	    return;
	}
...
Init_rmarshal()
{
    VALUE rb_mMarshal = rb_define_module("RMarshal");

    s_dumpas = rb_intern("_dump_as");
